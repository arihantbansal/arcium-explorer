/**
 * Arcium program error codes extracted from the v0.8.0 IDL.
 * Used by both the worker (tx-enricher) and frontend (computation detail, grid tooltip).
 */
...
export const ARCIUM_ERRORS: Record<number, ArciumError> = {
  6000: { name: "InvalidAuthority", msg: "The given authority is invalid" },
  6001: { name: "MxeKeysAlreadySet", msg: "The MXE keys are already set, i.e. all the nodes of the MXE cluster already agreed on the MXE keys" },
  6002: { name: "MxeKeysNotSet", msg: "The MXE keys are not set, i.e. not all the nodes of the MXE cluster agreed on the MXE keys" },
  ...
  6700: { name: "MxeNotInRecoveryState", msg: "MXE is not in recovery state" },
  6701: { name: "MxeAlreadyInRecovery", msg: "MXE is already in recovery state" },
  ...
  6716: { name: "MxeInRecoveryState", msg: "MXE is in recovery state, cannot queue new computations" },
};
